"""
Final Dataset-1 K=5 FedAvg training matrix (not a pilot).

Trains the frozen FedAvg contract on the existing final K=5 partitions across the
full grid: partition seeds {42, 43, 44} x conditions {iid, alpha_0p1, alpha_0p5,
alpha_1p0} = 12 runs. Every run uses batch size 4096, plain SGD at learning rate
0.1 (momentum 0, weight decay 0), one local epoch, and up to 40 communication
rounds with full client participation and sample-weighted FedAvg over the actual
client sizes n_k. Learning rate, batch size, and rounds are identical for every
condition; there is no per-alpha tuning.

All runs use one fixed training seed and start from one identical initial model
state built and saved here. Reads X_train/y_train (train) and X_val/y_val
(validation) only; never reads the test arrays. Checkpoint selection is validation
Macro-F1; the selected round may occur before round 40, which is only the fixed
maximum budget. Timings are simulation runtime, not communication latency.

Implementation follows the audited 27_final_fedavg_sgd_pilot.py; only the grid,
the fixed hyperparameters, the output roots, and the method/K provenance differ.
"""
...
RESULTS_DIR = Path("results/final_fedavg_k5")
MODELS_DIR = Path("models/final_fedavg_k5")
...
def run(partition_seed, condition, part_data, initial_state,
    ...
    return {"method": METHOD, "K": NUM_CLIENTS, "partition_seed": partition_seed,
...
def preflight_outputs() -> None:
    """Fail if any intended final output already exists."""
    ...
    if existing:
        ...
        raise RuntimeError(f"Refusing to run; final outputs already present:\n  {listing}")
