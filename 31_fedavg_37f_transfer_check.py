"""
FedAvg transfer/stability check on the corrected 37-feature Dataset-1 branch.

Tests whether the previously selected 41-feature FedAvg configuration transfers
stably to the 37-feature representation, using the previously selected 41-feature
configuration as a controlled transfer/stability candidate:
SGD lr 0.1, batch 4096, one local epoch, up to 40 rounds, full participation,
sample-weighted FedAvg over actual client sizes n_k, training/init seed 42, global
class-weighted CrossEntropyLoss from the unchanged y_train, and validation Macro-F1
checkpoint selection. This is a transfer/stability check, not hyperparameter tuning:
nothing is retuned for 37 features.

Runs exactly one condition: seed 44 / alpha_0p1, as a single severe non-IID
stress/transfer check, on the existing frozen K=5 partition index files. Reads
only the 37-feature train/validation arrays
and the label mapping; never reads the test arrays. Implementation follows the
audited 29_train_final_fedavg.py; only the input representation (37 features), the
single stress condition, and the isolated output roots differ.
"""
...
RESULTS_DIR = Path("results/fedavg_37f_transfer_check")
MODELS_DIR = Path("models/fedavg_37f_transfer_check")
...
TRANSFER_CONDITIONS = [(44, "alpha_0p1")]
...
def run(partition_seed, condition, part_data, initial_state,
    ...
    tag = f"fedavg37f_k{NUM_CLIENTS}_seed{partition_seed}_{condition}"
    ...
    return {"method": METHOD, "representation": "37f", "K": NUM_CLIENTS,
            "partition_seed": partition_seed, "condition": condition,
            "training_seed": TRAIN_SEED, "batch_size": BATCH_SIZE, "lr": LR,
            "max_rounds": MAX_ROUNDS, "best_round": best["round"],
            "best_val_macro_f1": best["macro_f1"],
...
def preflight_outputs() -> None:
    """Fail if any intended transfer-check output already exists."""
    intended = [INIT_PATH, CLASS_WEIGHTS_PATH, RESULTS_DIR / "transfer_summary.csv"]
    for partition_seed, condition in TRANSFER_CONDITIONS:
        tag = f"fedavg37f_k{NUM_CLIENTS}_seed{partition_seed}_{condition}"
    ...
    if existing:
        ...
        raise RuntimeError(f"Refusing to run; transfer-check outputs already present:\n  {listing}")
...
def main() -> None:
    ...
    # Confirm the 37-feature representation width for train and validation.
    ...
    # Load and verify each partition once (one per condition).
    part_cache = {}
    for partition_seed, condition in TRANSFER_CONDITIONS:
    ...
    for partition_seed, condition in TRANSFER_CONDITIONS:
        ...
        print(f"[fedavg37f_k{NUM_CLIENTS}_seed{partition_seed}_{condition}] DONE "
    ...
    assert states_equal(saved_init, initial_state), "initial state changed during the transfer check"

    assert len(summary_rows) == len(TRANSFER_CONDITIONS), "unexpected number of runs"
    pd.DataFrame(summary_rows).to_csv(RESULTS_DIR / "transfer_summary.csv", index=False)
    print(f"Wrote summary: {RESULTS_DIR / 'transfer_summary.csv'}")
