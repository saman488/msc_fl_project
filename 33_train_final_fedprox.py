"""
Final Dataset-1 K=5 FedProx training matrix (not a pilot).

FedProx extension of 29_train_final_fedavg.py. The FedAvg contract is preserved:
model, final K=5 partitions, partition seeds {42, 43, 44} x conditions {iid,
alpha_0p1, alpha_0p5, alpha_1p0} = 12 runs, class weights from the complete
y_train, plain SGD at learning rate 0.1 (momentum 0, weight decay 0), batch size
4096, one local epoch, up to 40 rounds with full client participation, the
per-client seed scheme, sample-weighted aggregation over the actual client sizes
n_k, validation metrics, macro-F1 checkpoint selection, timing decomposition,
partition assertions and provenance.
...
mass used by 29, so total equals task plus proximal at both client and round
level; round values combine the class-weight mass across participating clients.

The shared initial global model and the FedAvg class weights are read-only inputs
from the completed FedAvg K=5 run; the class weights are checked against freshly
computed full-y_train float32 weights before training. Reads train and validation
arrays only. Checkpoint selection is validation Macro-F1; the selected round may
occur before round 40, which is only the fixed maximum budget. Timings are
simulation runtime, not communication latency.

Outputs go only under results/final_fedprox_k5/ and models/final_fedprox_k5/,
...
# Read-only inputs produced by the completed FedAvg K=5 run.
FEDAVG_MODELS_DIR = Path("models/final_fedavg_k5")
FEDAVG_RESULTS_DIR = Path("results/final_fedavg_k5")
INIT_PATH = FEDAVG_MODELS_DIR / "initial_global_model.pt"
FEDAVG_CLASS_WEIGHTS_PATH = FEDAVG_RESULTS_DIR / "class_weights.npy"

# FedProx-only outputs.
RESULTS_DIR = Path("results/final_fedprox_k5")
MODELS_DIR = Path("models/final_fedprox_k5")
...
def run(partition_seed, condition, mu, part_data, initial_state,
    ...
    for rnd in range(1, MAX_ROUNDS + 1):
        ...
        for client_id in range(NUM_CLIENTS):
            # train_loss_client_* keeps 29's meaning: the weighted task loss.
    ...
    return {"method": METHOD, "mu": mu, "K": NUM_CLIENTS, "partition_seed": partition_seed,
...
def preflight_outputs(mu: float) -> None:
    """Fail if any intended output for this mu already exists."""
    frag = mu_fragment(mu)
    ...
    if existing:
        ...
        raise RuntimeError(f"Refusing to run; FedProx outputs already present:\n  {listing}")


def main() -> None:
    parser = argparse.ArgumentParser(description="Final Dataset-1 K=5 FedProx training matrix.")
    ...
    # Verify the FedAvg class-weight vector (read-only) against the fresh computation.
    assert FEDAVG_CLASS_WEIGHTS_PATH.exists(), f"FedAvg class weights not found: {FEDAVG_CLASS_WEIGHTS_PATH}"
    ...
    # Record the verified vector under the FedProx results root (source untouched).
    np.save(RESULTS_DIR / f"class_weights_mu{frag}.npy", weight_f32)

    # Reuse the shared initial state produced by the FedAvg run (read-only).
    assert INIT_PATH.exists(), f"Shared initial state not found: {INIT_PATH}"
    ...
    print(f"method=FedProx mu={mu} K={NUM_CLIENTS} lr={LR} momentum={MOMENTUM} "
