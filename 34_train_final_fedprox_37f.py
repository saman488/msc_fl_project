"""
Corrected 37-feature Dataset-1 K=5 FedProx final training matrix (not a pilot).

This is the FedProx counterpart of 32_train_final_fedavg_37f.py. Script 32 is
authoritative for every behaviour the two methods share: model, corrected
37-feature Dataset-1 representation (data/processed_37f, produced by
30_build_dataset1_37feature_branch.py), final K=5 partitions, partition seeds
{42, 43, 44} x conditions {iid, alpha_0p1, alpha_0p5, alpha_1p0} = 12 runs, class
weights from the complete y_train, plain SGD at learning rate 0.1 (momentum 0,
weight decay 0), batch size 4096, one local epoch, up to 40 rounds with full
client participation, the per-client seed scheme, sample-weighted aggregation over
the actual client sizes n_k, validation metrics, macro-F1 checkpoint selection,
timing decomposition, partition assertions and provenance. Learning rate, batch
size, and rounds are identical for every condition; there is no per-alpha or
additional tuning.

The only change is the proximal term, whose strength is set by a non-negative
--mu. After the current global state is loaded into a client, a detached copy of
that client's trainable named parameters is held fixed for the local epoch, and
each minibatch backpropagates

    total_loss = criterion(logits, labels) + 0.5 * mu * sum_i ||w_i - w_i_ref||^2

The penalty is computed unconditionally, so mu=0 adds an exact zero and reduces
to FedAvg. Server aggregation is unchanged.

Task, proximal and total losses accumulate on-device against the class-weight
mass used by 32, so total equals task plus proximal at both client and round
level; round values combine the class-weight mass across participating clients.

The shared initial global model and the class weights are read-only inputs from
the completed 37-feature FedAvg K=5 run (models/final_fedavg_k5_37f,
results/final_fedavg_k5_37f). The class weights are checked against freshly
computed full-y_train float32 weights, and the initial state is checked to have
the exact parameter shapes of a 37-input model, before training starts. The
41-feature FedProx results (results/final_fedprox_k5, models/final_fedprox_k5)
are retained unchanged as the original-feature baseline and are never written to.

Reads X_train/y_train (train) and X_val/y_val (validation) of the 37-feature
branch only; never reads the held-out arrays. Both are asserted to have exactly
37 columns. Checkpoint selection is validation Macro-F1; the selected round may
occur before round 40, which is only the fixed maximum budget. Timings are
simulation runtime, not communication latency.

Outputs go only under results/final_fedprox_k5_37f/ and models/final_fedprox_k5_37f/,
with the mu fragment in every filename so different mu values cannot collide.
"""
...
# FedProx 37-feature outputs only.
RESULTS_DIR = Path("results/final_fedprox_k5_37f")
MODELS_DIR = Path("models/final_fedprox_k5_37f")
...
def run(partition_seed, condition, mu, part_data, initial_state,
    ...
    frag = mu_fragment(mu)
    tag = f"fedprox37f_k{NUM_CLIENTS}_mu{frag}_seed{partition_seed}_{condition}"
    ...
    for rnd in range(1, MAX_ROUNDS + 1):
        ...
        for client_id in range(NUM_CLIENTS):
            # train_loss_client_* keeps 32's meaning: the weighted task loss.
    ...
    config = {
        ...
        "proximal_reference": "detached trainable named parameters of the global state, fixed per client epoch",
        "K": NUM_CLIENTS,
    ...
    return {"method": METHOD, "representation": REPRESENTATION, "input_dim": INPUT_DIM,
            "mu": mu, "K": NUM_CLIENTS, "partition_seed": partition_seed,
            "condition": condition, "training_seed": TRAIN_SEED, "batch_size": BATCH_SIZE,
...
def preflight_outputs(mu: float) -> None:
    """Fail if any intended 37-feature FedProx output for this mu already exists."""
    frag = mu_fragment(mu)
    ...
    if existing:
        ...
        raise RuntimeError(f"Refusing to run; 37-feature FedProx outputs already present:\n  {listing}")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Corrected 37-feature Dataset-1 K=5 FedProx training matrix.")
    parser.add_argument("--mu", type=float, required=True,
                        help="Non-negative FedProx proximal strength.")
    args = parser.parse_args()
    mu = float(args.mu)
    assert mu >= 0.0, "--mu must be non-negative"
    assert np.isfinite(mu), "--mu must be finite"
    frag = mu_fragment(mu)
    ...
    # Record the verified vector under the 37f FedProx results root (source untouched).
    ...
    print(f"method=FedProx representation={REPRESENTATION} input_dim={INPUT_DIM} mu={mu} "
          ...
          f"seeds={PARTITION_SEEDS} conditions={CONDITIONS} processed_dir={PROCESSED_DIR} "
    ...
    for partition_seed in PARTITION_SEEDS:
        for condition in CONDITIONS:
            ...
            print(f"[fedprox37f_k{NUM_CLIENTS}_mu{frag}_seed{partition_seed}_{condition}] DONE "
