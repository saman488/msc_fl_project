"""
Dataset-1 37-feature final held-out test evaluation.

Evaluates the 37 validation-selected models: 1 centralised, 12 FedAvg,
12 FedProx at mu=1e-5, 12 SCAFFOLD. Selection is re-derived from the saved
histories, summaries and configs; the test set is never used to choose anything.
...
Every federated row carries partition_id, partition_seed and condition, so the
results join directly to results/fl_partitions/final_partitions/partition_summary.csv
on partition_id. The centralised row has no partition and does not join.
"""
...
OUT_DIR = Path("results/final_test_37f")

CENTRAL_RESULTS = Path("results/centralised_corrected_37f")
CENTRAL_MODELS = Path("models/centralised_corrected_37f")
FEDAVG_RESULTS = Path("results/final_fedavg_k5_37f")
FEDAVG_MODELS = Path("models/final_fedavg_k5_37f")
FEDPROX_RESULTS = Path("results/final_fedprox_k5_37f")
FEDPROX_MODELS = Path("models/final_fedprox_k5_37f")
SCAFFOLD_RESULTS = Path("results/final_scaffold_k5_37f")
SCAFFOLD_MODELS = Path("models/final_scaffold_k5_37f")

PARTITION_SUMMARY = Path("results/fl_partitions/final_partitions/partition_summary.csv")
...
TRAINER_SCRIPTS = {
    "Centralised": Path("37_train_central_mlp_37f.py"),
    "FedAvg": Path("32_train_final_fedavg_37f.py"),
    "FedProx": Path("34_train_final_fedprox_37f.py"),
    "SCAFFOLD": Path("35_train_final_scaffold_37f.py"),
}
...
# Algorithm-identifying settings, taken verbatim from the saved configs.
ALGORITHM_CONFIG = {
    "FedAvg": {"method": "FedAvg"},
    "FedProx": {
        "method": "FedProx",
        "mu": FEDPROX_MU,
        "mu_fragment": FEDPROX_MU_FRAGMENT,
...
def check_config(config, algorithm, seed, condition, best_round, best_macro_f1,
                 models_dir, tag) -> None:
    """Confirm the saved config identifies this Dataset-1 run and its algorithm."""
...
def select_central() -> dict:
    ...
    for key in ("reloaded_val_macro_f1", "reload_macro_f1_delta",
                "holdout_evaluation_split_used", "script_name", "script_sha256",
                "epochs_run", "stopped_epoch", "stop_reason", "max_epochs"):
        require(key in summary.columns, f"centralised: summary is missing '{key}'")

    # This run early-stops, so the epoch count is whatever the run reached rather
    # than a fixed cap; the history must cover exactly the epochs that ran.
    stopped_epoch = int(row["stopped_epoch"])
    require(str(row["stop_reason"]) in ("patience_exhausted", "epoch_cap_reached"),
            f"centralised: stop_reason is {row['stop_reason']!r}")
    require(stopped_epoch <= int(row["max_epochs"]),
            f"centralised: stopped_epoch {stopped_epoch} exceeds max_epochs "
            f"{int(row['max_epochs'])}")
    require(int(row["epochs_run"]) == stopped_epoch,
            f"centralised: epochs_run {int(row['epochs_run'])} != stopped_epoch {stopped_epoch}")

    history = pd.read_csv(history_path)
    require(len(history) == stopped_epoch,
            f"{history_path}: expected {stopped_epoch} epochs, found {len(history)}")
    require(history["epoch"].tolist() == list(range(1, stopped_epoch + 1)),
            f"{history_path}: epochs are not 1..{stopped_epoch} in order")
...
def check_selection() -> list[dict]:
    ...
    # The FedProx directory holds all six mu values, so a loose pattern would pull
    # in 72 checkpoints instead of 12; confirm only mu=1e-5 was taken.
    fedprox = [m for m in selected if m["method"] == "FedProx"]
    require(len(fedprox) == 12,
            f"expected 12 FedProx checkpoints, found {len(fedprox)}")
    require(all(f"mu{FEDPROX_MU_FRAGMENT}_" in m["checkpoint_file"] for m in fedprox),
            "a FedProx checkpoint outside mu=1e-5 was selected")
    require(all(m["mu"] == FEDPROX_MU for m in fedprox),
            "a FedProx config records a mu other than 1e-5")

    # Every federated row must match a partition_summary row, since the Results
    # section joins on partition_id; an unmatched id would silently drop a run.
    require(PARTITION_SUMMARY.exists(), f"missing partition summary: {PARTITION_SUMMARY}")
    known = set(pd.read_csv(PARTITION_SUMMARY)["partition_id"])
    federated_ids = [m["partition_id"] for m in selected if m["partition_id"] is not None]
    require(len(federated_ids) == 36,
            f"expected 36 federated rows, found {len(federated_ids)}")
    missing = sorted(set(federated_ids) - known)
    require(not missing, f"partition_id values absent from {PARTITION_SUMMARY}: {missing}")
    return selected
...
def save_results(selected, results, class_names, device, out_dir=OUT_DIR) -> None:
    ...
    pd.DataFrame([{k: m[k] for k in
                   ("method", "partition_seed", "condition", "partition_id", "mu",
    ...
    for model, result in zip(selected, results):
        identity = {"method": model["method"], "partition_seed": model["partition_seed"],
                    "condition": model["condition"], "partition_id": model["partition_id"],
                    "mu": model["mu"], "tag": model["tag"],
    ...
    config = {
        ...
        "models_by_method": pd.Series([m["method"] for m in selected]).value_counts().to_dict(),
        ...
        "join_key": "partition_id (federated rows only; the centralised row has none)",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Dataset-1 37-feature final held-out test evaluation.")
    ...
    for model in selected:
        label = model["method"] if model["partition_seed"] is None else \
            f"{model['method']} seed{model['partition_seed']} {model['condition']}"
    ...
    for model in selected:
        ...
        print(f"  {model['tag']:<44} test_macro_f1={result['overall']['macro_f1']:.6f}",
              flush=True)
