"""
Corrected 37-feature Dataset-1 K=5 FedAvg final training matrix.

This is the corrected-feature counterpart of 29_train_final_fedavg.py. It runs the
same final K=5 FedAvg matrix on the corrected 37-feature Dataset-1 representation
(data/processed_37f, produced by 30_build_dataset1_37feature_branch.py), across the
full grid: partition seeds {42, 43, 44} x conditions {iid, alpha_0p1, alpha_0p5,
alpha_1p0} = 12 runs.

The configuration is the transferred setup that passed the severe 40-round
stability gate (31_fedavg_37f_transfer_check.py): SGD lr 0.1, batch 4096, one local
epoch, up to 40 rounds, full participation, sample-weighted FedAvg over the actual
client sizes n_k, training/init seed 42, global class-weighted CrossEntropyLoss from
the unchanged y_train, and validation Macro-F1 checkpoint selection. Learning rate,
batch size, and rounds are identical for every condition; there is no per-alpha or
additional tuning.

Outputs are written to isolated 37-feature roots (results/final_fedavg_k5_37f,
models/final_fedavg_k5_37f). The previous 41-feature results (results/final_fedavg_k5,
models/final_fedavg_k5) are retained unchanged as the original-feature baseline.

Reads X_train/y_train (train) and X_val/y_val (validation) of the 37-feature branch
only; never reads the test arrays. Checkpoint selection is validation Macro-F1; the
selected round may occur before round 40, which is only the fixed maximum budget.
Timings are simulation runtime, not communication latency.
"""
...
# Full grid: 3 labels x 5 conditions = 15 runs against the FEDARTML-partitioner set
# at data/fl_clients/hd_selected_fedartml/k_5. Seeds here are the directory labels
# seed_1/2/3, not the generating seeds; the mapping from label to generating seed is
# in that directory's seed_mapping.csv.
...
def run(partition_seed, condition, part_data, initial_state,
    ...
    return {"method": METHOD, "representation": REPRESENTATION, "K": NUM_CLIENTS,
...
def preflight_outputs() -> None:
    """Fail if any intended 37-feature final output already exists."""
    ...
    if existing:
        ...
        raise RuntimeError(f"Refusing to run; 37-feature final outputs already present:\n  {listing}")
