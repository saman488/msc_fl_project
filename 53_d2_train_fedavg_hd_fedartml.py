...
Runs on the partitions written by 51_d2_select_partitions_by_hd_fedartml.py, which
uses FedArtML's dirichlet_method and selects by measured Hellinger distance. The
FedAvg algorithm is preserved exactly: the same MLP, a K=5 partition grid over
directory labels {1, 2, 3} x conditions
...
Inputs (read-only):
    ...
    data/nf_cse_cic_ids2018_v2/fl_clients/hd_selected_fedartml/k_5/seed_*/<condition>/

Outputs go only under results/nf_cse_cic_ids2018_v2/final_fedavg_k5_hd_fedartml/
and models/nf_cse_cic_ids2018_v2/final_fedavg_k5_hd_fedartml/; the script refuses to
...
PART_ROOT = Path("data/nf_cse_cic_ids2018_v2/fl_clients/hd_selected_fedartml/k_5")
RESULTS_DIR = Path("results/nf_cse_cic_ids2018_v2/final_fedavg_k5_hd_fedartml")
MODELS_DIR = Path("models/nf_cse_cic_ids2018_v2/final_fedavg_k5_hd_fedartml")
...
# Full grid: 3 labels x 5 conditions = 15 runs against the FEDARTML set at
# data/nf_cse_cic_ids2018_v2/fl_clients/hd_selected_fedartml/k_5, written by
# 51_d2_select_partitions_by_hd_fedartml.py. Seeds here are the directory labels
# seed_1/2/3, not the generating seeds; that mapping is in the set's seed_mapping.csv.
