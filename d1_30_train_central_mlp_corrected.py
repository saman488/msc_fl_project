...
NF-UNSW-NB15-v2 (Dataset 1).

This script is a corrected re-implementation of ``04_train_central_mlp.py``.
The model contract (41 -> 128 -> 64 -> 10), the training contract
(AdamW, lr=1e-3, weight_decay=1e-5, batch_size=4096, 20 epochs, seed 42) and
the balanced class weighting (weight[c] = N / (C * n_c)) are preserved exactly.
...
6.  Isolated outputs.  Nothing produced by the original centralised run is read
    or overwritten; the script refuses to start if any of its own intended
...
MODELS_DIR = Path("models/centralised_corrected")
RESULTS_DIR = Path("results/centralised_corrected")
CONFIGS_DIR = Path("configs/centralised_corrected")
