"""
Dataset-2 (NF-CSE-CIC-IDS2018-v2) K=5 FedAvg final training matrix.

Dataset-2 counterpart of 32_train_final_fedavg_37f.py. The FedAvg algorithm is
preserved exactly: the same MLP, the same final K=5 partition grid over partition
seeds {42, 43, 44} x conditions {iid, alpha_0p1, alpha_0p5, alpha_1p0} = 12 runs,
...
identical for every condition; there is no per-alpha or additional tuning.
...
Outputs go only under results/nf_cse_cic_ids2018_v2/final_fedavg_k5/ and
models/nf_cse_cic_ids2018_v2/final_fedavg_k5/; the script refuses to start if any
intended output already exists.
...
RESULTS_DIR = Path("results/nf_cse_cic_ids2018_v2/final_fedavg_k5")
MODELS_DIR = Path("models/nf_cse_cic_ids2018_v2/final_fedavg_k5")
