...
X_TRAIN_PATH = PROJECT_ROOT / "data" / "processed_37f" / "X_train.npy"
...
def main() -> None:
    ...
    # Protect against accidentally writing a different partition from the one verified.
    target_hd = 0.50
    expected_hd = 0.496503472259
    if not np.isclose(hd, expected_hd, rtol=0.0, atol=1e-12):
        raise RuntimeError(
            f"Unexpected HD {hd:.12f}; verified value was {expected_hd:.12f}"
        )
