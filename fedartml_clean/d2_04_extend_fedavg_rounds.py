...
TRAINER_SOURCE = ROOT / "d2_04_train_fedavg.py"

EXPECTED_TRAINER_SHA256 = (
    "7c9175da360432088c0e4ad33930eeb125af4a7624b88a83e05fa8fc22cab61e"
)

VALID_CONDITIONS = ["iid", "hd_0p25", "hd_0p5", "hd_0p75", "hd_0p90"]
...
def load_trainer(train_seed: int, partition_root: Path):
    ...
    spec = importlib.util.spec_from_file_location(
        "verified_d2_fedavg_trainer", TRAINER_SOURCE
    )
    trainer = importlib.util.module_from_spec(spec)
    sys.modules["verified_d2_fedavg_trainer"] = trainer
...
def run_one(args):
    ...
    tag = f"fedavg_k{trainer.NUM_CLIENTS}_seed{seed}_{condition}"
    ...
    if device.type == "cuda":
        resident = {
            "x_train": torch.from_numpy(
                np.load(trainer.PROCESSED_DIR / "X_train.npy")
            ).to(torch.float32).to(device),
            "y_train": torch.from_numpy(
                np.load(trainer.PROCESSED_DIR / "y_train.npy")
            ).to(torch.long).to(device),
            "x_val": torch.from_numpy(
                np.load(trainer.PROCESSED_DIR / "X_val.npy")
            ).to(torch.float32).to(device),
            "y_val": torch.from_numpy(
                np.load(trainer.PROCESSED_DIR / "y_val.npy")
            ).to(torch.long).to(device),
        }

        assert tuple(resident["x_train"].shape) == (
            trainer.EXPECTED_TRAIN_ROWS,
            trainer.INPUT_DIM,
        )
        assert tuple(resident["x_val"].shape) == (
            trainer.EXPECTED_VAL_ROWS,
            trainer.INPUT_DIM,
        )
        assert int(resident["y_train"].numel()) == trainer.EXPECTED_TRAIN_ROWS
        assert int(resident["y_val"].numel()) == trainer.EXPECTED_VAL_ROWS
        assert resident["x_train"].dtype == torch.float32
        assert resident["x_val"].dtype == torch.float32
        assert resident["y_train"].dtype == torch.long
        assert resident["y_val"].dtype == torch.long

        val_loader = trainer.ResidentValLoader(
            resident["x_val"],
            resident["y_val"],
            4096,
        )
    ...
    if resident is not None:
        client_indices_cuda = [
            torch.from_numpy(ci.astype(np.int64)).to(device)
            for ci in part_data["client_indices"]
        ]
        assert len(client_indices_cuda) == trainer.NUM_CLIENTS
        for client_id, client_indices in enumerate(client_indices_cuda):
            assert int(client_indices.numel()) == sizes[client_id]
    ...
    for rnd in range(last_round + 1, args.end_round + 1):
        ...
        for client_id in range(trainer.NUM_CLIENTS):
            ...
            # Preserve the D2 trainer's round/client RNG logic.
            local_seed = (
                trainer.TRAIN_SEED + rnd * 100 + client_id
            )

            torch.manual_seed(local_seed)
            trainer.seed_accelerator(local_seed)
            ...
            if resident is not None:
                position_loader = DataLoader(
                    trainer.LocalPositionDataset(sizes[client_id]),
                    batch_size=trainer.BATCH_SIZE,
                    shuffle=True,
                    num_workers=0,
                    generator=generator,
                )
                loader = trainer.ResidentClientBatches(
                    position_loader,
                    client_indices_cuda[client_id],
                    resident["x_train"],
                    resident["y_train"],
                    device,
                )
...
def main():
    ...
    p.add_argument(
        "--end-round", type=int, default=100
    )
    p.add_argument(
    ...
    print("D2 CONVERGENCE EXTENSION COMPLETE", flush=True)
