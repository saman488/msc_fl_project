...
PROCESSED_DIR = ROOT / "data" / "nf_cse_cic_ids2018_v2" / "processed"
LABEL_MAP = (
    ROOT
    / "configs"
...
PART_ROOT = ROOT / "fedartml_clean" / "d2_partitions" / "k_5"
FINAL_ROOT = ROOT / "fedartml_clean" / "d2_convergence_extension_100"
OUT_DIR = ROOT / "fedartml_clean" / "d2_final_test_100r"
...
INPUT_DIM = 36
NUM_CLASSES = 7
EXPECTED_TEST_ROWS = 2_817_634
EXPECTED_TEST_CLASS_COUNTS = np.array(
    [2_495_335, 6_186, 18_137, 208_540, 72_600, 16_311, 525],
    dtype=np.int64,
)
EXPECTED_X_TEST_SHA256 = (
    "fd4928fd7213728bcd9cea17881195cac31589c6e95ecb5c7a61135e666559ca"
)
EXPECTED_Y_TEST_SHA256 = (
    "5705d4fefbf836a527702984126f5898dfe579426ce0c943dfafb3b17b9fd2a4"
)
BATCH_SIZE = 4096
...
def load_trainer():
    spec = importlib.util.spec_from_file_location("d2_clean_eval_trainer", TRAINER)
    trainer = importlib.util.module_from_spec(spec)
    sys.modules["d2_clean_eval_trainer"] = trainer
...
def load_class_names():
    ...
    require(
        ...
        "label mapping is not exactly classes 0..6",
    )
...
def select_models(seeds):
    ...
    for seed in seeds:
        require(
            seed in CONDITIONS_BY_SEED,
            f"unsupported D2 partition seed: {seed}",
        )
        ...
        for condition in CONDITIONS_BY_SEED[seed]:
            ...
            require(
                int(manifest.get("K", -1)) == 5,
                f"{tag}: partition manifest K is not 5",
            )

            # Non-IID FedArtML manifests store the aggregate HD under
            # fedartml_hellinger_distance. The verified D2 IID manifests
            # store the same RMS pairwise aggregate under hd_pairwise_rms.
            achieved_hd = manifest.get("fedartml_hellinger_distance")

            if achieved_hd is None:
                achieved_hd = manifest.get("hd_pairwise_rms")

            require(
                achieved_hd is not None,
                f"{tag}: partition manifest contains no usable HD value",
            )
...
def main():
    ...
    require(
        len(selected) == 9,
        "FINAL TEST REFUSED: expected exactly 9 locked D2 models",
    )
    ...
    # Bind the held-out arrays to the exact preprocessing lineage already
    # verified against the RunPod train/validation files.
    require(
        x_test_path.is_file(),
        f"missing D2 held-out test file: {x_test_path}",
    )
    require(
        y_test_path.is_file(),
        f"missing D2 held-out test file: {y_test_path}",
    )
    require(
        file_sha256(x_test_path) == EXPECTED_X_TEST_SHA256,
        "D2 X_test SHA256 does not match verified preprocessing lineage",
    )
    require(
        file_sha256(y_test_path) == EXPECTED_Y_TEST_SHA256,
        "D2 y_test SHA256 does not match verified preprocessing lineage",
    )
    ...
    require(
        ...
        "y_test does not contain exactly classes 0..6",
    )

    observed_test_counts = np.bincount(
        y_test.astype(np.int64, copy=False),
    ...
    require(
        ...
        "D2 y_test class counts do not match verified preprocessing lineage",
    )
    ...
    metadata = {
        ...
        "partition_seeds": sorted(args.seeds),
        "training_seed": TRAIN_SEED,
        ...
        "test_access_rule":
            "test loaded only after all locked D2 model selections passed preflight",
    }
    ...
    print(f"WROTE FINAL D2 TEST RESULTS: {OUT_DIR}")
